...
export const REMOVE_FROM_EVENT_MAIN_LIST_SELECT = "embed:manage_event:remove_main_select";

export const remove_from_event_main_list = async ({ event, guild }: { event: EventSchema; guild: Guild }) => {
    const participantIds = event.mainListParticipant;

    if (!participantIds || participantIds.length === 0) {
        return [
            new ContainerBuilder()
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent("## Удалить из основного состава"),
                )
                .addSeparatorComponents(
                    new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true),
                )
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(`${botAssetsEmojis.dot} Основной состав пуст`),
                ),
        ];
    }
                ...
                new ActionRowBuilder<MessageActionRowComponentBuilder>()
                    .addComponents(
                        new StringSelectMenuBuilder()
                            .setCustomId(`${REMOVE_FROM_EVENT_MAIN_LIST_SELECT}:${event.id}`)
                            .setMinValues(1)
