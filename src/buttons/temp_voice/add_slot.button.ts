...
export default {
    ...
    async execute(interaction: ButtonInteraction) {
        ...
        const meta = metaBuilder(member, { button: "temp_voice_add_slot" })
        ...
        try {
            ...
            if (newLimit > 99) {
                log.button.info(meta, `Limit is already at 99 - aborting`)
                await interaction.editReply({ embeds: [new EmbedBuilder().setTitle("Ошибка").setDescription("> Лимит участников 99")] });
                return;
            }
            ...
            log.button.fatal(meta, `Unhadled erro while trying to increase limit for temp voice users: ${error}`)
            console.error(error);
