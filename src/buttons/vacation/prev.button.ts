...
export default {
    customId: VACATION_LIST_PAGINATION_CUSTOM_ID.prev,
    dynamic: true,
    async execute(interaction: ButtonInteraction) {
        if (!interaction.isButton() || !interaction.inCachedGuild()) return;

        const meta = metaBuilder(interaction.member, {
            button: "vacation_list_prev",
        });

        try {
            const rawPage = interaction.customId.slice(VACATION_LIST_PAGINATION_CUSTOM_ID.prev.length + 1);
            ...
            log.button.error(meta, "Failed to handle vacation list prev");
            await safeReply(interaction, error, "vacation_list_prev.execute", interaction.id);
        }
    },
} satisfies Button;